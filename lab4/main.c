...
int check(char* path, char* delim)
{
    ...
    for (str_tmp = stok(buf_path, delim); str_tmp; str_tmp = stok(NULL, delim))
    {
        ...
        if ((th_wi_let - str_tmp == 1) || (th_wi_let - str_tmp == 2))
        {
            ...
            {
                if (str_tmp[0] > 90 || str_tmp[1] > 90)
                {  //проверка на заглавные буквы
                    return 1;
                }
            }
        }
        else if (((th_wi_let = sstr(str_tmp, "/")) - str_tmp) == 0)
        {  // linux?
            if (sspn(str_tmp, "*?:\\<\">,|"))
            {
                return 2;
...
char* process(char* path, char* delim)
{
    ...
    {  //делим на подстроки весь путь
        if (slen(res))
        {  //делитель для итоговой строки между подстроками
            scat(res, delim);
        }
        ...
        char* ttmp;
        int a = 0,
            b = 0;  //если есть буква в пути то а = 1, иначе если нет, то б = 1
        if ((ttmp = sstr(buf, ":\\")))
        {
            a = 1;
        }
        ...
        {                                                        // windows
            ...
            if (a)
            {
                ...
                c = 3;
                if (n == 1)
                {  //если одна буква в пути
                    tr[10] = (char)tolower(buf[0]);
                }
                else if (n == 2)
                {  //если две буквы в пути
                    tr[10] = (char)tolower(buf[0]);
            ...
            for (tmp_t = stok(buf + c, "\\"); tmp_t; tmp_t = stok(NULL, "\\"))
            {  //добавление в итоговую подстроку путей исходного путя
                scat(tr, "/");
                scat(tr, tmp_t);
            }
